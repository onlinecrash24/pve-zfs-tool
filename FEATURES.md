...
- **checkzfs Health Panel** -- Runs `checkzfs --source <ip>` on the target and renders an OK/WARN/CRIT summary plus a grouped table; ANSI-stripped, replicated-only filter on by default. The replica search is confined to the pair's target dataset (`--replicafilter`), and on a same-host pair the target subtree is kept out of the source set too -- without that, checkzfs saw every dataset twice (via SSH and locally, identical snapshot GUIDs) and reported each one as a broken replica of itself
...
- **Import of existing configs** -- A config that grew outside the tool -- upstream's default `zsync.conf`, or a hand-named file -- is listed as importable rather than hidden. Import shows what it would do first (old path, new `<source-ip>.conf` path, source, target, and any cron entry found in root's crontab **or** `/etc/cron.d`), then writes the new file with every value and unknown key carried over, takes over the schedule and removes the old entry, and renames the old file with an `.imported-<timestamp>` suffix. It never overwrites an existing pair, and with two cron entries for one file it touches neither
...
- **Webhook** -- One JSON document per event to any HTTP endpoint. The body is an editable template with placeholders (`{{title}}`, `{{severity}}`, `{{state_code}}`, `{{key}}`, ...) that is parsed as JSON and filled in inside string values, so a quote in a message can never break it, and a placeholder that is the whole value keeps its type. A generic document and a Slack shape ship as starting templates; Teams, Discord, n8n or a monitoring bridge need only a different template. Paired events (host offline/online, pool failed/recovered) share one correlation key and carry `new`/`resolved`, so a receiver can close the alert it opened
