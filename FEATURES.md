...
- **Webhook** -- One JSON document per event to any HTTP endpoint. The body is an editable template with placeholders (`{{title}}`, `{{severity}}`, `{{state_code}}`, `{{key}}`, ...) that is parsed as JSON and filled in inside string values, so a quote in a message can never break it, and a placeholder that is the whole value keeps its type. Slack, SIGNL4 and a Nagios-style monitoring shape ship as starting templates; Teams, Discord, n8n or a Checkmk bridge need only a different template. Every request is signed with HMAC-SHA256 (`X-PVEZFS-Signature`) when a secret is set; paired events (host offline/online, pool failed/recovered) share one correlation key and carry `new`/`resolved`, so a receiver can close the alert it opened
