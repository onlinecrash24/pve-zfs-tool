...
- **Bulk-Restore je Kategorie** -- Ein Klick spielt alle Dateien einer Kategorie zurück (z. B. alle APT-Quellen **und** Signing-Keyrings zusammen); vorhandene Dateien werden übersprungen, sofern Überschreiben nicht aktiviert ist
- **Pakete nachinstallieren** -- Die gesicherte `dpkg --get-selections` anwenden (nur install/hold — additiv, entfernt nie) via `dpkg --set-selections` + `apt-get dselect-upgrade`, als Hintergrund-Task mit Live-Fortschritt. Eine Vorab-Prüfung stellt sicher, dass die APT-Quellen + Keyrings aus dem Backup bereits auf dem Ziel liegen, und warnt sonst vor dem Start (fehlende Keys lassen den ganzen Lauf mit NO_PUBKEY scheitern); ein fehlgeschlagener Lauf mit Repo-/Signaturfehlern bekommt einen Klartext-Hinweis über dem apt-Log
