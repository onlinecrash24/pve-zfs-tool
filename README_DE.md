...
- **Drei Haupt-Aktionen, in dieser Reihenfolge** -- (1) **Pakete nachinstallieren**, (2) **Alle Configs wiederherstellen**, (3) **Alle Gast-Configs wiederherstellen** — der empfohlene Ablauf, direkt ganz oben
- **Pakete nachinstallieren (in sich abgeschlossen)** -- Stellt **zuerst** die APT-Quellen + Signing-Keys aus dem Backup wieder her (damit Drittanbieter-Pakete auflösbar sind), **dann** die gesicherte `dpkg --get-selections` (nur install/hold — additiv, entfernt nie) via `dpkg --set-selections` + `apt-get dselect-upgrade`, als Hintergrund-Task mit Live-Fortschritt. Meldet **ehrlich**, welche angeforderten Pakete danach noch nicht installiert sind (`dpkg --set-selections` ignoriert unbekannte Namen still, ein apt-Exit 0 bedeutet also nicht, dass alles installiert wurde) — die Liste der noch fehlenden Pakete und das apt-Log werden angezeigt
...
- **Bulk-Restore je Kategorie** -- Jede Kategorie-Überschrift hat zusätzlich ihr eigenes „Alle wiederherstellen" für feinere Kontrolle; vorhandene Dateien werden übersprungen, sofern Überschreiben nicht aktiviert ist
