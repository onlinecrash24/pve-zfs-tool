...
- **Webhook** -- Je Ereignis ein JSON-Dokument an einen beliebigen HTTP-Endpunkt. Der Body ist eine editierbare Vorlage mit Platzhaltern (`{{title}}`, `{{severity}}`, `{{state_code}}`, `{{key}}`, …), die als JSON geparst und innerhalb der String-Werte gefüllt wird — ein Anführungszeichen in einer Nachricht kann sie nie zerbrechen, und ein Platzhalter, der den ganzen Wert bildet, behält seinen Typ. Slack, SIGNL4 und eine Nagios-artige Monitoring-Form liegen als Startvorlagen bei; Teams, Discord, n8n oder eine Checkmk-Brücke brauchen nur eine andere Vorlage. Mit gesetztem Geheimnis ist jede Anfrage per HMAC-SHA256 signiert (`X-PVEZFS-Signature`); Ereignispaare (Host offline/online, Pool defekt/erholt) teilen sich einen Korrelationsschlüssel und tragen `new`/`resolved`, so dass ein Empfänger den Alarm schließen kann, den er geöffnet hat
