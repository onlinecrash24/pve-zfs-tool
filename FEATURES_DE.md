...
- **checkzfs-Statuspanel** -- Führt `checkzfs --source <ip>` auf dem Ziel aus und zeigt eine OK/WARN/CRIT-Übersicht plus gruppierte Tabelle; ANSI bereinigt, Filter „nur replizierte" standardmäßig aktiv. Die Replikatsuche ist auf das Ziel-Dataset des Paars begrenzt (`--replicafilter`), und beim Paar auf derselben Maschine bleibt der Zielbaum auch aus der Quellmenge heraus -- ohne das sah checkzfs jeden Datensatz doppelt (per SSH und lokal, identische Snapshot-GUIDs) und meldete jeden als kaputtes Replikat seiner selbst
...
- **Import bestehender Configs** -- Eine Config, die außerhalb des Tools gewachsen ist -- Upstreams `zsync.conf` oder eine von Hand benannte Datei -- wird als importierbar gelistet statt ausgeblendet. Der Import zeigt zuerst, was er täte (alter Pfad, neuer `<quell-ip>.conf`-Pfad, Quelle, Ziel und jeder Cron-Eintrag aus der root-crontab **oder** `/etc/cron.d`), schreibt dann die neue Datei mit allen Werten und unbekannten Schlüsseln, übernimmt den Zeitplan und entfernt den alten Eintrag, und benennt die alte Datei mit dem Suffix `.imported-<Zeitstempel>` um. Ein bestehendes Paar wird nie überschrieben, und bei zwei Cron-Einträgen für eine Datei bleibt beides unangetastet
...
- **Webhook** -- Je Ereignis ein JSON-Dokument an einen beliebigen HTTP-Endpunkt. Der Body ist eine editierbare Vorlage mit Platzhaltern (`{{title}}`, `{{severity}}`, `{{state_code}}`, `{{key}}`, …), die als JSON geparst und innerhalb der String-Werte gefüllt wird — ein Anführungszeichen in einer Nachricht kann sie nie zerbrechen, und ein Platzhalter, der den ganzen Wert bildet, behält seinen Typ. Ein generisches Dokument und eine Slack-Form liegen als Startvorlagen bei; Teams, Discord, n8n oder eine Monitoring-Brücke brauchen nur eine andere Vorlage. Ereignispaare (Host offline/online, Pool defekt/erholt) teilen sich einen Korrelationsschlüssel und tragen `new`/`resolved`, so dass ein Empfänger den Alarm schließen kann, den er geöffnet hat
